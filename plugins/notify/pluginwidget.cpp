...
void PluginWidget::paintEvent(QPaintEvent *e)
{
    ...
    QPixmap pixmap;
    QString iconName = ":/icons/resources/icons/normal.svg";
    int iconSize;
    const Dock::DisplayMode displayMode = qApp->property(PROP_DISPLAY_MODE).value<Dock::DisplayMode>();

    if (displayMode == Dock::Efficient) {
        iconName = ":/icons/resources/icons/normal.svg";
        iconSize = 16;
    } else {
        iconName = ":/icons/resources/icons/fashion.svg";
...
const QPixmap PluginWidget::loadSvg(const QString &fileName, const QSize &size) const
{
    ...
    return QIcon(fileName).pixmap(size, devicePixelRatioF());
}
